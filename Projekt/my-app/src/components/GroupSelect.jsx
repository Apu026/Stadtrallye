...
const GroupSelect = () => {
  ...
  const [takenGroups, setTakenGroups] = useState([]); // Gruppen, die schon vergeben sind
  const navigate = useNavigate();

  // Hilfsfunktion zum Mischen (Fisher-Yates)
  ...
  useEffect(() => {
    // Lade alle Gruppennamen aus der DB (API-Call)
    ...
    // Lade bereits vergebene Gruppennamen für diesen Raum (API-Call, optional)
  ...
  const handleSubmit = async (e) => {
    ...
    // API-Call, um Gruppe im Raum zu registrieren
    try {
  ...
  return (
    <div className="groupselect-bg">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" style={{ width: '100%', padding: 16, fontSize: 22, borderRadius: 12, background: '#1976d2', color: '#fff', border: 'none', marginTop: 8, fontWeight: 700, letterSpacing: 1 }}>
          Weiter
        </button>
      </form>
    </div>
