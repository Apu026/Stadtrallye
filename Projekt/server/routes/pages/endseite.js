...
// Returns sessiongroups optionally filtered by roomCode query
router.get('/sessiongroups', async (req, res) => {
  const { roomCode } = req.query;
  // Keep response shape consistent with domain router
  ...
  try {
    ...
    console.error('Error fetching sessiongroups for endseite:', e && e.message ? e.message : e);
    if (e && e.stack) console.error(e.stack);
    // Fall back to safe empty array to avoid breaking the frontend
